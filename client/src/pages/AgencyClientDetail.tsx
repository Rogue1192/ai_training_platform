...
function ContentPublishPanel({ campaignId }: { campaignId: number }) {
    ...
    <div className="space-y-3">
      ...
      {allUrlsEntered && (
        <div className="flex items-center gap-2 rounded-md bg-green-500/10 border border-green-500/30 px-3 py-2">
          ...
          <p className="text-xs text-green-300 font-medium">All pages published — indexing in progress.</p>
        </div>
      )}
  ...
  return (
    <Card>
      <CardContent className="py-3 px-4 space-y-3">
        ...
        {/* Content Publish Panel — shown when content is ready for agency to add to client site */}
        {(campaign.status === 'publishing' || campaign.status === 'indexing' || campaign.status === 'indexing_verification') && (
          <div className="border-t pt-3">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
              <FileText className="h-3.5 w-3.5" />
              Content Publishing
            </p>
            <ContentPublishPanel campaignId={campaign.id} />
          </div>
        )}
