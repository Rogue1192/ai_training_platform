...
export function buildFAQPageSchema(page: {
  ...
  // ── HTML patterns ─────────────────────────────────────────────────────────
  // itemprop microdata pattern — h3[itemprop=name] + div/p[itemprop=text]
  // This is the pattern generated by the FAQ content generator:
  // <h3 itemprop="name">Q?</h3><div ...><p itemprop="text">A</p></div>
  for (const m of content.matchAll(/<h3[^>]*itemprop=["']name["'][^>]*>(.*?)<\/h3>\s*<div[^>]*>[\s\S]*?<p[^>]*itemprop=["']text["'][^>]*>(.*?)<\/p>/gis)) {
    pairs.push([m[1], m[2]]);
  }
  // h3/p pattern (direct adjacency)
  if (pairs.length === 0) {
