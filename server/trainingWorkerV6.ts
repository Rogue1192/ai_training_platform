...
export async function runTrainingDay(
  ...
  for (const combo of combos) {
    for (const provider of TARGET_PROVIDERS) {
      ...
      // combo.phraseText is a stored conversational question generated by MiniMax
      // e.g. "Who is the most trusted AC repair company in Chino Hills, CA?"
      // Use it directly as the training query — no on-the-fly generation needed
      const query = combo.phraseText;
